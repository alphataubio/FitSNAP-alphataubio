...
class CTildeBasisSet:
  ...
  def blist(self):
    ...
    blist = []
    if not self.bzeroflag: blist = [f"{t} [0]" for t in self.elements]
  ...
  def basis_ranks(self):
    ...
    ranks = []
    if not self.bzeroflag: ranks.extend([0] * self.nelements)
    ...
    for mu in sorted(self.functions.keys()):
      ...
      for func in sorted_funcs: ranks.append(int(func.rank))
  ...
  def save_yaml(self, filename):
    ...
    with open(filename, 'w') as f:
      ...
      for (i, j), spec in sorted_bonds:
        f.write(f"  [{i}, {j}]:\n")
        d = spec.to_dict()
        for k in ["nradmax", "lmax", "nradbasemax", "radbasename"]: write_indent(f, 2, k, d[k])
  ...
  def trim_basis_by_mask(self, mask):
    ...
    for mu in sorted(self.functions.keys()):
      ...
      for func in ordered_funcs:
        if mask[mask_idx]: kept_funcs.append(func)
        mask_idx += 1

...
CACHE_FILE = os.path.join(lib_path, "wigner_cache.json")
...
class WignerCacheManager:
    ...
    def load(self):
        if False and os.path.exists(CACHE_FILE):
            try:
                with open(CACHE_FILE, "rb") as f: self.cache = json.load(f)
    ...
    def save(self):
        ...
        try:
            with open(CACHE_FILE, "wb") as f: json.dump(self.cache, f)
...
# =========================================================
# SECTION 7: RPI BASIS VECTOR GENERATION (NEW)
# =========================================================

def get_rpi_basis_vectors(mu_in, n_in, l_in, local_wigner_cache=None):
    """
    Calculates independent PA-RPI basis vectors using generalized Wigner coefficients.
    
    This function calculates the actual basis vector coefficients for a given
    shell defined by (mu, n, l). It ensures linear independence via SVD/Gram-Schmidt.
    
    Args:
        mu_in, n_in, l_in: lists of indices defining the shell.
        local_wigner_cache: dict for caching w3j symbols (optional).
    
    Returns:
        list of dict: Each dict represents an independent basis function with keys:
            'label': canonical label string
            'vector': numpy array of coefficients (size = num_m_configs)
            'm_configs': list of m-tuples corresponding to vector indices
            'mus', 'ns', 'ls': the shell indices
    """
    if local_wigner_cache is None:
        local_wigner_cache = {}
    ...
    # 2. Generate Candidate Couplings (L-trees)
    # We use BasisGenerator to get valid coupling schemes
    combined_labs = BasisGenerator.get_canonical_labels(tuple(n_in), tuple(l_in), tuple(mu_in), L_R=0)
    if not combined_labs:
        return []

    # 3. Setup Vector Space (M-configurations)
    rank = len(l_in)
    # Parse the first label to get sorted reference for permutations
    ref_mu, ref_n, ref_l, _ = parse_label_full(combined_labs[0], rank)
    combined = sorted(list(zip(ref_mu, ref_n, ref_l)))
    ref_full_sorted = list(zip([x[0] for x in combined], [x[1] for x in combined], [x[2] for x in combined]))
    ref_l_sorted = [x[2] for x in combined]
    
    # Generate all valid m states summing to 0
    m_configs = _generate_m_states_iterative(ref_l_sorted)
    if not m_configs:
        return []
    ...
    for label in combined_labs:
        # Parse current label to extract intermediate Ls (the tree structure)
        mu_leaves, n_leaves, l_leaves, l_inters = parse_label_full(label, rank)
        curr_full = list(zip(mu_leaves, n_leaves, l_leaves))
        
        # Find permutations mapping reference shell to current shell
        perm_maps = get_constrained_permutations(ref_full_sorted, curr_full)
        if not perm_maps:
            continue
        ...
        # Calculate Generalized Wigner Coefficient for each M-config
        for m_ref in m_configs:
            val = np.float64(0.0)
            # Symmetrize over valid permutations (PA-RPI requirement)
            for indices in perm_maps:
                m_curr = [m_ref[i] for i in indices]
                # Use internal calculation function
                w = calculate_generalized_wigner_exact(l_leaves, l_inters, m_curr, local_wigner_cache)
                val += w
            w_vec.append(val)
            if abs(val) > 1e-15:
                is_zero_vec = False
        ...
        if not is_zero_vec:
            ...
            if norm > 1e-15:
                valid_candidates.append({
                    'label': label,
                    'vector': vec / norm,
                    'm_configs': m_configs, # Shared reference
                    'mus': mu_leaves,
                    'ns': n_leaves,
                    'ls': l_leaves
                })
    ...
    # 5. SVD / Gram-Schmidt Selection for Independence
    # Construct matrix A from candidate vectors
    A = np.column_stack([c['vector'] for c in valid_candidates])
    ...
    basis_functions = []
    basis_matrix = [] # Orthogonal basis vectors stored here
    
    for i, candidate in enumerate(valid_candidates):
        vec = candidate['vector']
        
        # Gram-Schmidt Orthogonalization against selected basis
        if len(basis_matrix) == 0:
            basis_matrix.append(vec)
            basis_functions.append(candidate)
        else:
            vec_ortho = vec.copy()
            B = np.array(basis_matrix)
            # Double projection for stability
            for _ in range(2):
                projections = np.dot(B, vec_ortho)
                vec_ortho -= np.dot(projections, B)
                
            norm_ortho = np.linalg.norm(vec_ortho)
            if norm_ortho > tolerance:
                # Store the orthogonalized vector as the actual coefficient vector
                # This defines the RPI basis function
                candidate['vector'] = vec_ortho / norm_ortho
                basis_matrix.append(candidate['vector'])
                basis_functions.append(candidate)
                
        # Stop if we found enough functions (rank check)
        if len(basis_functions) >= n_expected:
             break

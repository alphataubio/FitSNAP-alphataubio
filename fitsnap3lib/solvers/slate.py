...
class SLATE(SlateCommon):
    ...
    def perform_fit_ard(self):
        ...
        for iter_ in range(self.max_iter):
            ...
            pt.debug_all_print(f"*** ok 8 - got sigma diagonal, size in KB: {sigma_diag.nbytes / 1e3:.2f} (saved {(n_active**2 * 8 / 1e6):.2f} MB by not returning full matrix)")
            
            # Map active coefficients back to full coefficient vector
            coef_ = np.zeros(n, dtype=np.float64)
            coef_[active_indices] = coef_active_
            pt.debug_all_print(f"*** ok 9 - mapped coefficients")
            ...
            pt.debug_all_print(f"*** ok 10 - computed SSE")

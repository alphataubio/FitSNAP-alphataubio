...
def _ensure_lattice_for_adios2(
  ...
  *,
  ...
  if not np.isfinite(det) or det <= 0:
    raise RuntimeError(f"_ensure_lattice_for_adios2: invalid cell after fix (det={det}, rank={np.linalg.matrix_rank(cell)})")
...
def _process_chunk(args):
  ...
  for i in range(start_idx, end_idx):
    ...
    cell, positions, forces, stress = _ensure_lattice_for_adios2( cell, positions, forces, stress,
      pad_angstrom=lattice_pad, min_side_angstrom=lattice_min_side,)
    ...
    if config.get('Spin', 1) == 1 and group_ok: configs.append(config)
...
def process_lmdb_dir(
  ...
  for i in range(0, dataset_size, chunk_size):
    ...
    chunks.append((start_idx, end_idx, group_name, allowed_elements, test_bool, lattice_pad, lattice_min_side, allowed_groups))
...
def process_dataset_path(
  ...
  print(f"\nProcessing {subset_type} subset: {len(lmdb_entries)} LMDB root(s), layout={layout}", file=sys.stderr)
  all_configs = []
...
def write_adios2_file(configs, output_path, allowed_elements):
  ...
  for i, config in enumerate(tqdm(configs, desc="  Preparing data", unit="config")):
    ...
    if has_forces:
      if 'Forces' in config: forces_list.append(config['Forces'].flatten())
      else: forces_list.append(np.zeros(config['NumAtoms'] * 3, dtype=np.float64))
  ...
  if has_nbo_charges:
    for config in configs:
      if 'nbo_charges' in config:
        ...
        if nbo.size != config['NumAtoms']: raise ValueError(f"nbo_charges length {nbo.size} != NumAtoms {config['NumAtoms']}")
  ...
  if np.any(~np.isfinite(dets)) or np.any(dets <= 0):
    ...
    raise RuntimeError(f"Lattice verification failed: {nbad}/{nconfigs} configs have det(Lattice) <= 0 or non-finite")
  print(f"  Lattice det check: min={float(np.min(dets)):.6g} max={float(np.max(dets)):.6g} (Å³, all > 0)", file=sys.stderr)
...
Expected directory structures:
  ...
  parser.add_argument('--lattice-pad', type=float, default=10.0, metavar='ANG',
                      help='Vacuum padding (Å) on each side when synthesizing a box from positions (OMOL / missing cell)')
  parser.add_argument('--lattice-min-side', type=float, default=8.0, metavar='ANG',
                      help='Minimum orthorhombic edge (Å) when synthesizing a cell from positions')
  parser.add_argument('--groups', nargs='+', default=None, metavar='NAME',
                      help=(
  ...
  if allowed_groups is None: print("Allowed groups (data_id / Group): all", file=sys.stderr)
  else: print(f"Allowed groups (data_id / Group): {', '.join(sorted(allowed_groups))}", file=sys.stderr,)
  print(
  ...
  if args.workers: print(f"Using {args.workers} parallel workers", file=sys.stderr)
  else: print(f"Using {mp.cpu_count()} parallel workers (all CPUs)", file=sys.stderr)
